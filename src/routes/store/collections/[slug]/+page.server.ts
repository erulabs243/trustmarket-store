...
export const load = (async ({params}) => {
  ...
  const collections = (await api.get(`collections?handle[]=${slug}`).json()) as CollectionsRes;
  ...
  const products = (await api.get(`products?collection_id[]=${collection.id}`).json()) as ProductRes;
  if(products.count === 0) {
